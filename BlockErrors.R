...
bes <- files[filter] %>% tolower() %>% str_detect(pattern = "/blockerrors?\\d*.txt")
counter <- 0
for (file in files[filter][bes]){
  counter <- counter + 1
  print(counter)
  print(file)
  data <- read_delim(file = file.path(path, file), delim = "\t", col_names = F) %>%
    ...
    mutate(X1 = str_remove(string = X1, pattern = ".*:") %>% str_trim(),
           ...
           X8 = ifelse(str_length(X8) > 2, X7, "") %>% str_trim(),
           X9 = str_remove(string = X9, pattern = ".*:") %>% str_trim(),
           X10 = str_trim(string = X10) %>%
             str_replace(pattern = "&#1605;&#1608;&#1580;&#1608;&#1583;&#1610; &#1581;&#1587;&#1575;&#1576; &#1705;&#1575;&#1601;&#1610; &#1606;&#1610;&#1587;&#1578;",
                         replacement = "موجودی حساب کافی نیست;") %>% str_trim(),
           fileName = file,
           Type = "blockerrors")
  if("finalData" %in% ls()) {
...
colnames(finalData) <- c("BankName", "AccountNumber", "ShebaNumber", "Amount", "BlockCode", "Date", "TransactionTime", "ReferenceCode", "ErrorCode", "Status", "FileName", "Type")
...
sqlSave(channel = connection, dat = finalData, tablename = paste0("bankLog.BlockError", folder), rownames = FALSE, append = T, fast = F)
odbcClose(channel = connection)

write_csv(x = finalData, file = paste0("blockerror", folder,".csv"))
